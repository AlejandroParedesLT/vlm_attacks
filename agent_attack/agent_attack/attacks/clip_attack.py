...
class ClipAttack:
    def __init__(self, epsilon=16 / 255, alpha=1 / 255, iters=500, size=None):
        self.epsilon = epsilon
        self.alpha = alpha
        self.iters = iters
        self.size = size
        ...
        self.models = [
            ClipFeatureExtractor(model_path="openai/clip-vit-base-patch32").eval().cuda().requires_grad_(False),
            ClipFeatureExtractor(model_path="openai/clip-vit-base-patch16").eval().cuda().requires_grad_(False),
            ClipFeatureExtractor(model_path="openai/clip-vit-large-patch14").eval().cuda().requires_grad_(False),
            ClipFeatureExtractor(model_path="openai/clip-vit-large-patch14-336").eval().cuda().requires_grad_(False)
        ]

        def index_fn(count, num_models=len(self.models), ssa_N=20):
            max_val = ssa_N * num_models
            count = count % max_val
            return count // ssa_N

        self.loss_fn = CLIPFeatureLoss(self.models, index_fn)
        self.attacker = SSA_CommonWeakness(
            self.models,
            epsilon=self.epsilon,
            step_size=self.alpha,
            total_step=self.iters,
            criterion=self.loss_fn,
            targeted_attack=True,
        )

    def forward(self, image, target_text, victim_text=None):
        if self.size is not None:
            image = resize_image(image, self.size)

        image = np.array(image).astype(np.float32) / 255.0
        image = torch.from_numpy(image).unsqueeze(0).permute(0, 3, 1, 2).cuda()

        self.loss_fn.set_ground_truth(target_text)
        if victim_text is not None:
            self.loss_fn.set_victim_text(victim_text)

        adv_dict = self.attacker(image, None)
        adv_images = {}
        for step, adv_x in adv_dict.items():
            arr = adv_x.squeeze(0).detach().cpu().numpy()
            arr = (arr * 255).astype("uint8").transpose(1, 2, 0)
            adv_images[step] = Image.fromarray(arr)
