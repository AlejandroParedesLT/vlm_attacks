...
def config() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        choices=["liuhaotian/llava-v1.5-7b", "Salesforce/instructblip-vicuna-7b","Qwen/Qwen2.5-VL-7B-Instruct"],
    ...
    for example in dataset:
        ...
        adv_image.save(
            os.path.join(
                ...
                "custom_duke",
                example["id"],
                f"{args.captioning_model}_{args.attack}_caption_attack_image.png",
            )
        )
        # Save the caption
        with open(
            os.path.join(
                ...
                "custom_duke",
                example["id"],
                f"{args.captioning_model}_{args.attack}_caption_attack_caption.txt",
            ),
            "w",
        ) as f:
            f.write(adv_caption)
        ...
        with open(
            os.path.join(
                ...
                "custom_duke",
                example["id"],
                f"{args.captioning_model}_{args.attack}_caption_attack_acc.txt",
            ),
            "w",
        ) as f:
            f.write(str(acc))
...
if __name__ == "__main__":
    ...
    if args.data_from_hub:
        ...
        examples = get_examples(os.path.join("exp_data", "custom_duke"))
        dataset = examples
