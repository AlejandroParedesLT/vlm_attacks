...
def test(args: argparse.Namespace, config_file_list: list[str], use_caption=True) -> None:
    ...
    # 'exp_data/step_agent_adv/classifieds_13_wrong_color_cap_2/config.json',
    # 'exp_data/step_agent_adv/reddit_142_wrong_text_cap_2/config.json',
    ...
    for config_file in config_file_list:
        ...
        try:
            ...
            # print(obs['text'])
            # quit(0)
            clean_obs = {key: value for key, value in obs.items()}
            clean_obs["text"] = obs["clean_text"]
            clean_state_info: StateInfo = {"observation": clean_obs, "info": info}
            trajectory.append(clean_state_info)
            ...
            for _ in range(steps):
                ...
                if args.agent_type == "search" and type(action) == list:
                        ...
                        try:
                            if args.value_function in ["gpt4o"]:
                                ...
                                score = value_function.evaluate_success(
                                    ...
                                    obs_texts=last_obs_texts[-(args.max_depth + 1) :] + [obs["text"]],
                                )
                            else:
                    ...
                    while action_queue and search_counter < args.vf_budget:
                        ...
                        if next_action["action_type"] == ActionTypes.NONE:
                            ...
                            last_obs_texts.append(obs["text"])
                ...
                for best_idx, action in enumerate(best_actions):
                    ...
                    action_str = get_action_description(
                        action,
                        clean_state_info["info"]["observation_metadata"],
                        action_set_tag=args.action_set_tag,
                        prompt_constructor=agent.prompt_constructor if isinstance(agent, PromptAgent) else None,
                    )
                    ...
                    if curr_url != prev_url:
                        ...
                        prev_url = curr_url
                    clean_state_info = {"observation": clean_obs, "info": info, "url": env.page.url}
                    trajectory.append(clean_state_info)
